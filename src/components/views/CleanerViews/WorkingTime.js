...
const WorkingTime = () => {
  ...
  const searchDoneJobs = () => {
    if (startDate && endDate) {
      fetch(
        `http://localhost:8080/cleaner/1/date?startDate=${startDate}&endDate=${endDate}`,
        {
          method: "GET",
          headers: {
  ...
  return (
    <Stack alignItems={"center"} spacing={2}>
      ...
      />
      <Button color={"primary"} variant={"contained"} onClick={searchDoneJobs}>
        Check
      </Button>
      <Grid container spacing={2} wrap={"wrap"} justifyContent={"center"}>
        {doneJobs
          ? doneJobs.map((booking) => {
