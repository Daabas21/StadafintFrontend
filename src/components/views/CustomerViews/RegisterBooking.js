...
const RegisterBooking = () => {
  ...
  const time = `${timeValue?.$d?.toLocaleTimeString("en-GB", {
    hour: "2-digit",
    minute: "2-digit",
  })}:00`;

  const checkIfBookingExists = () => {
    filteredList = bookingList.filter(
      (book) =>
  ...
  const handleSave = (e) => {
    ...
    if (filteredList.length === 0) {
      ...
      alert(
        "You have already made a booking like this. Please try again with other date, time and/or address."
      );
    }
  };
